module QuickbooksModel
	...
	def handle_custom_type(value)
		value.to_a.each do |arr|
			persist("primary_contact", "#{arr['data_ext_value']}") if arr['data_ext_name'] == 'Site Contact'
			persist("primary_email", "#{arr['data_ext_value']}") if arr['data_ext_name'] == 'Site Email'
			persist("primary_phone", "#{arr['data_ext_value']}") if arr['data_ext_name'] == 'Site Phone'
		end
	end
