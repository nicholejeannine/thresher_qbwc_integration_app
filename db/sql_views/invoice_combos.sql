...
AS SELECT
   ...
   `P`.`URL` AS `URL`
FROM ((`Thresher`.`invoice_combos` `IC` join `Thresher`.`Payments` `P` on((`P`.`FK_qInvoices_PKEY` = `IC`.`qInvoices_PKEY`))) join `qb`.`receive_payments` `RP` on((`RP`.`invoice_id` = `IC`.`id`))) where (`P`.`Amount` = trim(trailing '000' from `RP`.`total_amount`));
...
FROM `RP`.`total_amount`) AS `QB Amount`,`P`.`Date` AS `Portal Date`,`RP`.`txn_date` AS `QB Date` from ((`Thresher`.`invoice_combos` `IC` join `Thresher`.`Payments` `P` on((`P`.`FK_qInvoices_PKEY` = `IC`.`qInvoices_PKEY`))) join `qb`.`receive_payments` `RP` on((`RP`.`invoice_id` = `IC`.`id`))) where (`P`.`Amount` = trim(trailing '000' from `RP`.`total_amount`)) order by `IC`.`qInvoices_PKEY`;